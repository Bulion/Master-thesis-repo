...
The robot used in this part is called Dobot Magician \cite{dobot}. This is simple desk robotic arm with 3 degrees of freedom and the possibility of replacing the effectors. All axis are driven by stepper motors so before use in needs firstly perform homeing action. It could be seen at figure \ref{fig:dobot}.
...
First, the task related to the visualization of sensory data was carried out. Based on the fact that reading information from the robot had already been developed, it was quite easy to get to it. After connection is estabilish and data is accquired script which was written is generating small spheres and pushes them away from the center of LiDAR at a distance read from the ROS. This give us visual feedback which objects could be seen by given sensor. In this case LiDAR beam could not detect glass doors (Figure \ref{fig:glassDoor}).
