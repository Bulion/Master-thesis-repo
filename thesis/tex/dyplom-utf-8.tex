...
Prepared elements allow to use shared workspace as an interactive display
...
In the case of mobile robots, the topics of visualization of sensory data and route planning were examined. Displaying in real time sensors reading in user environment could help with debuging some hardware or software issues. Also being able to see waypoints of robot on the floor could help to check if it will not hit any obstacle.
...
\subsection{Unity}
Application for Hololens could be written in every programming language with has support for Universal Windows Platform API. However, due to the fact that goggles are working in a 3D environment, developer also need a graphics engine to be able to create objects in 3D space. Here Unity comes with the help along with the whole set of libraries called HoloToolkit provided by Microsoft.
